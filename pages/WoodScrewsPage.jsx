...
const WoodScrewsPage = () => {
  ...
  const fetchProducts = (sortBy) => (dispatch) => {
    axios
      .get('https://perekrostok-backend-production-6738.up.railway.app/api/products?category=5')
      .then(({ data }) => {
  ...
  return (
    <>
      <Head>
        <title>Перекресток Лысково саморезы по дереву</title>
        <meta name="description" content="Магазин 'Перекресток' саморезы по дереву" />
        <meta property="og:title" content="Саморезы по дереву магазина 'Перекресток'" />
      </Head>
      <Header />
        ...
        <div className="section__products__container">
          <div className="section__products__nav__top">
            <span className="section__products__nav__top__left">
              <span className="section__products__nav__top__left-title">Саморезы по дереву</span>
            </span>
