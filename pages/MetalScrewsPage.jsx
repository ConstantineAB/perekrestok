...
const MetalScrewsPage = () => {
  ...
  const fetchProducts = (sortBy) => (dispatch) => {
    axios
      .get('https://perekrostok-backend-production-6738.up.railway.app/api/products?category=4')
      .then(({ data }) => {
  ...
  return (
    <>
      <Head>
        <title>Перекресток Лысково cаморез по металлу</title>
        <meta name="description" content="Магазин 'Перекресток' cаморез по металлу" />
        <meta property="og:title" content="Cаморез по металлу магазина 'Перекресток'" />
      </Head>
      <Header />
        ...
        <div className="section__products__container">
          <div className="section__products__nav__top">
            <span className="section__products__nav__top__left">
              <span className="section__products__nav__top__left-title">Cаморезы по металлу</span>
            </span>
...
export default MetalScrewsPage;
