...
const PuttyPage = () => {
  ...
  const fetchProducts = (sortBy, category) => (dispatch) => {
    axios
      .get('https://perekrostok-backend-production-6738.up.railway.app/api/products?category=2')
      .then(({ data }) => {
  ...
  return (
    <>
      <Head>
        <title>Перекресток Лысково шпаклевка</title>
        <meta name="description" content="Магазин 'Перекресток' шпаклевка" />
        <meta property="og:title" content="Шпаклевка магазина 'Перекресток'" />
      </Head>
      <Header />
        ...
        <div className="section__products__container">
          <div className="section__products__nav__top">
            <span className="section__products__nav__top__left">
              <span className="section__products__nav__top__left-title">Шпаклевка</span>
            </span>
